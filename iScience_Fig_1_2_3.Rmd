...
# Fig 2 - First-person pronoun frequency correlation with therapeutic alliance
...
# Fig 3 - Disfluency speech marker correlation with therapeutic alliance

```{r figure 3, echo = FALSE}
# Again, separate the matrix into different datasets for "AUX-INTJ" and "nonfluent-LIWC" features.
...
# Summarize the group-level frequencies of disfluency categorized by roles in the session, i.e. therapist and patient
...
# Run Spearman method partial correlation correcting for Number of Visits in the dyad (for non-parametric AUX-INTJ data) and pearson method correlation (for parametric nonfluency data) to correlate disfluency features with patient alliance.
...
cor.test(logFreq, Patient.Alliance,
         alternative="two.sided",method="pearson", conf.level=0.95)
```
# Fig 4 - Relationship between patients' trust game behavior and therapists' personal pronoun features / Mediaton analysis
